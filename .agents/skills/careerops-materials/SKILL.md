...
CareerOps is required for verified resume and cover-letter generation. This Skill routes work to the pinned external project; it does not copy or replace CareerOps rules. Job Search Ops adds an opinionated, reusable personal-rule layer on top of CareerOps.
...
Then read every path in `materials.ruleFiles` from the selected Job Search Ops home's `.jobops/config.json`. During setup, store a file-backed personal Skill or rule document with `jobops setup --home <home> --material-rules <path>`. If the host exposes an existing personal resume Skill, prefer configuring that Skill's `SKILL.md` instead of copying it. Apply precedence in this order: current explicit user instruction, configured personal rule files, the built-in Job Search Ops resume defaults, then general CareerOps guidance. Record conflicts and overrides in the artifact audit.
...
4. Build the structured request and run `jobops material prepare --request <file>`. Job Search Ops injects the built-in and configured `ruleFiles`; the result must remain a draft
