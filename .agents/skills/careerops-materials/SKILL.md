...
1. Run `career-journal doctor` and read `config/dependency-manifest.yml`. Resolve the configured CareerOps root without embedding a developer-specific path
...
4. Build the structured request and run `career-journal material prepare --request <file>`. CAREER JOURNAL injects the built-in and configured `ruleFiles`; the result must remain a draft
5. Run `career-journal material verify --request <file>` and the relevant rendered-file capability. A generated file, clean page count, or successful process exit does not prove factual or visual compliance
...
Record an application as submitted when the user explicitly says it was submitted. Record a submitted resume or cover letter only when the exact artifact is identified; use `career-journal material mark-submitted --id <application> --file <path> --confirm`. Do not assume the newest draft was uploaded or invent an application date.

For a workspace created by v0.1.0-alpha.5 or earlier, read `.jobops/config.json` and accept an explicitly configured `jobops-adapter.mjs` only as upgrade compatibility. Do not discover the legacy adapter for a new workspace. New workspaces and integrations use `.career-journal/config.json`, `career-journal`, and `career-journal-adapter.mjs`.
