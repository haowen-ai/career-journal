...
1. In Agent-managed mode, use the host's existing mail capability first. On macOS, attempt discovery before asking any mailbox setup question: inspect accessible Apple Mail or other host mail accounts, show the addresses, and ask which one or more accounts the user uses for job search. Do not ask for IMAP host names, usernames, passwords, or environment-variable names when the host can already read those accounts.
