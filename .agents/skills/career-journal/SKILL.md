...
3. Configure every selected address as a read-only `host` account and use a stable connector name such as `apple-mail`. After the host integration visibly confirms the address, record a trusted-host verification with `email verify-host`. One `mail-sync` task must contain the full selected account set. The Agent itself acts as the host mail connector in Agent-managed mode; no separate Apple Mail adapter is required. Before building a batch, read the saved cursor from `email list` and copy it exactly into `beforeCursor`. Read every message received during the complete rolling previous 24 hours using a received-time window only. Do not prefilter by sender, company, role, subject, recruiting terms, or known applications. Paginate through all result pages, include every message, and deduplicate by stable provider message ID. Write one bounded read-only host batch per account with coverage mode `rolling-24h-all-messages`, the actual window bounds, `allMessages: true`, and `paginationComplete: true`, then import it with `email sync-host`. Enabled Jev must receive each imported message before CAREER JOURNAL decides whether it is job-search related. Do not advance the cursor or call `automation run` until every page and batch import succeeds.
4. Jev is optional and never blocks core onboarding. Reuse it when a configured capability is already available. Otherwise configure `--model-provider host-agent`; the current coding Agent supplies semantic decisions while mailbox and automation setup continues. After core onboarding passes `doctor`, make one optional Jev offer. If the user declines, skips it, or has no access, keep `host-agent` active. Once enabled, Jev is the primary semantic decision engine and evaluates every message in the rolling 24-hour batch first, including messages that are ultimately classified as non-recruiting. If Jev is unavailable, errors, or returns an invalid or low-confidence result, try the configured structured LLM, then local rules, and finally manual review. If the user chooses Jev, help them use the official [TypeSafe Agent Skill](https://github.com/typesafe-ai/skills/tree/main/skills/typesafe-ai) and console. Never ask the user to paste, send, or provide an API key or secret in chat or an Agent prompt. On macOS, store a newly created key in Keychain and configure `keychain:career-journal-typesafe:<local-account>`; otherwise use another local private credential path and store only a secret reference.
...
- Email: in Agent mode, read the complete rolling previous 24 hours from every selected host account without keyword prefiltering, then use `email verify-host` and `email sync-host`; use IMAPS for standalone mode; manual EML is one-off fallback only
...
- Jev: when enabled, evaluate every message in the rolling 24-hour batch with Jev first; fall back to the configured structured LLM, local rules, and then manual review

CareerOps is the independently maintained MIT-licensed [career-ops-hq/career-ops](https://github.com/career-ops-hq/career-ops) project. CAREER JOURNAL routes material work through the repository's `careerops-materials` adapter and must preserve upstream attribution. The TypeSafe Agent Skill is independently maintained by [TypeSafe AI](https://github.com/typesafe-ai/skills) under MIT. It guides the Jev integration but is not copied into this repository. Tracking remains available without Jev. When Jev is enabled it evaluates every message in the rolling 24-hour batch first; configured language models, local rules, and manual review provide the fallback path.
