...
export function createServer({ db, config, webRoot, artifactRoot, revealFile }) {
  ...
  return http.createServer(async (request, response) => {
    try {
      ...
      if (url.pathname.startsWith('/api/')) {
        ...
        if (!await handleApi(request, response, url, { db, config, artifactRoot, revealFile })) sendJson(response, 404, { error: 'Not found' });
        return;
      }
