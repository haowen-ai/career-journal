...
"$node_bin" "$cli" application add --home "$data_home" --company UpgradeCo --role PreservedRole
"$node_bin" "$cli" event record --home "$data_home" --id upgradeco-preservedrole --event-id upgrade-event --type application_update --title Received --observed-at 2026-09-19T01:00:00Z --recorded-at 2026-09-19T02:00:00Z
printf 'draft artifact\n' >"$scratch/resume.txt"
"$node_bin" "$cli" artifact add --home "$data_home" --id upgradeco-preservedrole --kind resume --lifecycle draft --file "$scratch/resume.txt"
"$node_bin" "$cli" automation configure --home "$data_home" --task local-backup --time 23:00 --timezone UTC --enabled
...
"$node_bin" "$cli" migrate --home "$data_home" --dry-run
"$node_bin" "$cli" migrate --home "$data_home" --apply
"$node_bin" "$cli" application show --home "$data_home" --id upgradeco-preservedrole >"$scratch/application.json"
...
"$node_bin" "$cli" automation list --home "$data_home" | grep -q 'jobops-local-backup'
