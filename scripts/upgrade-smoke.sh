...
if test -f "$checkout/bin/career-journal.mjs"; then
  source_cli="$checkout/bin/career-journal.mjs"
  "$node_bin" "$source_cli" setup --home "$data_home" --timezone UTC --email-provider host --email-address candidate@school.edu --email-connector upgrade-smoke
else
  source_cli="$checkout/bin/jobops.mjs"
  "$node_bin" "$source_cli" setup --home "$data_home" --timezone UTC --skip-email
fi
if test -f "$data_home/.career-journal/config.json"; then
  config_path="$data_home/.career-journal/config.json"
else
  config_path="$data_home/.jobops/config.json"
fi
"$node_bin" "$source_cli" application add --home "$data_home" --company UpgradeCo --role PreservedRole
"$node_bin" "$source_cli" event record --home "$data_home" --id upgradeco-preservedrole --event-id upgrade-event --type application_update --title Received --observed-at 2026-09-19T01:00:00Z --recorded-at 2026-09-19T02:00:00Z
printf 'draft artifact\n' >"$scratch/resume.txt"
"$node_bin" "$source_cli" artifact add --home "$data_home" --id upgradeco-preservedrole --kind resume --lifecycle draft --file "$scratch/resume.txt"
"$node_bin" "$source_cli" automation configure --home "$data_home" --task local-backup --time 23:00 --timezone UTC --enabled
"$node_bin" "$source_cli" automation list --home "$data_home" >"$scratch/source-automations.json"
automation_id=$("$node_bin" -e "const fs=require('node:fs');const rows=JSON.parse(fs.readFileSync(process.argv[1]));const row=rows.find((item)=>item.type==='local-backup');if(!row)process.exit(1);process.stdout.write(row.id)" "$scratch/source-automations.json")
"$node_bin" -e "const fs=require('node:fs');const p=process.argv[1];const c=JSON.parse(fs.readFileSync(p));c.model={provider:'openai-compatible',baseUrl:'https://example.test/v1',model:'test-model',secretRef:'env:MODEL_KEY'};c.jev={accessState:'enabled',model:'jev-latest',secretRef:'env:JEV_KEY',mode:'shadow'};fs.writeFileSync(p,JSON.stringify(c,null,2)+'\\n',{mode:0o600})" "$config_path"
...
test -f "$backup_dir/career-journal.db" || test -f "$backup_dir/jobops.db"
...
"$node_bin" "$primary_cli" automation run --home "$data_home" --id "$automation_id" --dry-run >/dev/null
...
"$node_bin" -e "const fs=require('node:fs');const rows=JSON.parse(fs.readFileSync(process.argv[1]));const row=rows.find((item)=>item.type==='local-backup');if(!row||row.id!==process.argv[2]||row.schedule!=='22:45')process.exit(1)" "$scratch/automations.json" "$automation_id"
grep -q 'env:MODEL_KEY' "$config_path"
grep -q 'env:JEV_KEY' "$config_path"
